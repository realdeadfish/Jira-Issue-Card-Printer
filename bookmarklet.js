(function() {
  ...
  function fillCard(card, data) {
    ...
    //Tags
    if (data.tags) {
      card.find(".issue-tags").text(data.tags.reduce(function(p, c) { return p + ", " + c; }));
    } else {
  ...
  function printPreviewElement() {
    ...
    result.find("#tags-checkbox").click(function() {
      ...
      if (global.settings.hideTags == false && global.settings.hideEpic == false) {
    ...
    result.find("#epic-checkbox").click(function() {
      ...
      // can only display one of these two
      if (global.settings.hideEpic == false && global.settings.hideTags == false) {
